...
import { Arrow, Lic } from "@/coms/icon";
const menuStateHandler = () => {
  document.querySelectorAll(".menu").forEach((ele) => {
    if (ele.getAttribute("data-link") === m.route.get()) {
      ele?.classList.add("active", "pointer-events-none");
    } else {
      ele?.classList.remove("active", "pointer-events-none");
    }
  });
};
        ...
        <div class="w-34 min-h-full px-1 select-none ">
          ...
          <ul>
            <li>
              ...
              <ul class="">
                <li
                  data-link="/lic/list/all"
                  onclick={(e) =>
                    m.route.set(e.target.getAttribute("data-link"), {})
                  }
